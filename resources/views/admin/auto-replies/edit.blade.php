<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Auto-Reply Configuration') }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.auto-replies.update', $autoReply) }}" method="POST">
                        ...
                        <div class="mb-4">
                            <label for="trigger" class="block text-sm font-medium text-gray-700">
                                Trigger (Kata Kunci) *
                            </label>
                            <input type="text" name="trigger" id="trigger" value="{{ old('trigger', $autoReply->trigger) }}" required
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="response" id="response" rows="6" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('response', $autoReply->response) }}</textarea>
                            @error('response')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="priority" id="priority" value="{{ old('priority', $autoReply->priority) }}" min="0" max="1000" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            @error('priority')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            <label class="flex items-center">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $autoReply->is_active) ? 'checked' : '' }}
                        ...
                        <div class="mb-4">
                            <label class="flex items-center">
                                <input type="checkbox" name="case_sensitive" value="1" {{ old('case_sensitive', $autoReply->case_sensitive) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('admin.auto-replies.index') }}" class="mr-4 text-gray-600 hover:text-gray-900">
                                Cancel
                            </a>
